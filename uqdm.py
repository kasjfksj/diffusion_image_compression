...
class VDM_SD_Net(torch.nn.Module):
    """VDM using SD 1.5 UNet for eps_hat prediction with optional Fourier features."""
    ...
    def __init__(self, config):
        ...
        if mcfg.use_fourier_features:
            self.fourier_features = FourierFeatures()
            total_ch = mcfg.n_channels * (1 + self.fourier_features.num_features)
            self.conv_in = nn.Conv2d(total_ch, self.unet.config.in_channels, 1)
        
        # Time embedding adapter
        self.time_proj = nn.Linear(mcfg.embedding_dim, self.unet.config.cross_attention_dim)
        
        # Output projection
        out_ch = mcfg.n_channels * 2 if mcfg.get('learned_prior_scale') else mcfg.n_channels
    ...
    def forward(self, z, g_t):
        ...
        # Prepare input
        h = torch.cat([z, self.fourier_features(z)], dim=1) if self.mcfg.use_fourier_features else z
        if hasattr(self, 'conv_in'):
            h = self.conv_in(h)

        # UNet forward (unconditioned)
        encoder_hidden_states = self.time_proj(t_emb).unsqueeze(1)
        h = self.unet(h, timestep=t * 1000, encoder_hidden_states=encoder_hidden_states).sample
        h = self.conv_out(h)
        if self.mcfg.get('learned_prior_scale'):
            eps_hat, scale = torch.split(h, self.mcfg.n_channels, dim=1)
            scale = torch.nn.functional.softplus(scale + self.SOFTPLUS_INV1)
            return eps_hat + z, scale
        return h + z
...
def load_checkpoint(path, use_sd=False):
    """
